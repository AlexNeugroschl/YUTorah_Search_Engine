...
class DataPreprocessing:
    ...
    def __get_user_stats(self):
        ...
        self.user_stats_df = listens_df.merge(downloads_df, on='user').merge(teacher_df, on='user')
    ...
    def __get_top_teacher(self, shiur_stats_df: pd.DataFrame) -> pd.DataFrame:
        ...
        merged_df['name'] = merged_df['teacher_title'] + ' ' + merged_df['first_name'] + ' ' + merged_df['last_name']
        merged_df.drop(columns=['teacher_title', 'last_name', 'first_name'], inplace=True)
        most_common_name = merged_df.groupby(['user'])['name'].agg(pd.Series.mode)
        shiur_stats_df['top_teach'] = most_common_name
        return shiur_stats_df
    
    def __get_downloads(self, shiur_stats_df: pd.DataFrame):
        downloads_df = self.bookmakrs_df[self.bookmakrs_df['downloaded'] == 1].groupby('user')
        shiur_stats_df['total_downloads'] = downloads_df['downloaded'].count()
        return shiur_stats_df

    def __get_listens(self, shiur_stats_df: pd.DataFrame):
        listened_df = self.bookmarks_df[self.bookmarks_df['played'] == 1].groupby('user')
